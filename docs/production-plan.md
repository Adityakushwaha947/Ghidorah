...
| P3. ModelClient adapter | Lease-fenced Postgres dispatch journal and OpenRouter adapter implemented; live acceptance passed on `z-ai/glm-4.7` pinned to one upstream | Mastra integration so the fixture runs on the real route; OpenAI adapter; live refusal and disconnect cases; tokenizer-based input reservation; USD ledger; finite test budget per run |

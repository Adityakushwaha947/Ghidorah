...
- The CLI and integration-test npm command have patch prechecks. The native reproduction checks directly. Since the 19 September hardening increment, backend execution also verifies the accepted installed bundle hashes before allocating a run. Importing the backend is not a patch installer.

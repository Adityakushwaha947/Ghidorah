import { z } from "zod";
import { FindingSchema as WireFindingSchema, VerificationSchema, VerificationReceiptSchema, type Finding, type FindingClaim } from "../contracts/findings.js";
import { validateVersion } from "./contracts.js";
...
export const FindingSchema = WireFindingSchema.superRefine((finding, context) => {
  ...
  if (receipt && (receipt.candidateRevision !== candidateRevision(finding) || receipt.subjectDigest !== subjectDigest(finding))) {
...
const independentVerdict = VerificationSchema.extend({
  result: z.enum(["pass", "fail"]), method: VerificationSchema.shape.method.unwrap(), artifactRef: z.string().min(1),
  receipt: VerificationReceiptSchema, review: z.never().optional(),
});

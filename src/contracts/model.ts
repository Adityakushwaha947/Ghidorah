...
export const ModelUsageSchema = z.strictObject({ inputTokens: z.number().int().nonnegative(), outputTokens: z.number().int().nonnegative() });
...
export const ModelMessageSchema = z.union([
  ...
  z.strictObject({ role: z.literal("assistant"), content: z.string(), toolCalls: z.array(ModelToolCallSchema).optional() }),
...
export const ModelRequestSchema = z.strictObject({
  seamVersion: z.literal(SEAM_VERSION.model), requestId: text, model: text, messages: z.array(ModelMessageSchema).min(1),
  tools: z.array(ModelToolSchema), maxOutputTokens: PositiveIntegerSchema,
  sampling: z.strictObject({ temperature: z.number().finite().optional(), seed: z.number().int().optional() }).optional(),
  ...
  for (const message of request.messages) {
    if (message.role !== "tool" && pending.size) reject("Tool results must resolve preceding calls before another message.");
    ...
    } else if (message.role === "tool" && !pending.delete(message.callId)) reject("Tool result has no unique preceding call.");
  }
...
export const ModelResponseSchema = z.strictObject({
  requestId: text, model: text, content: z.string(), toolCalls: z.array(ModelToolCallSchema),
  finishReason: z.enum(["stop", "tool_calls", "length", "refusal"]), usage: ModelUsageSchema,
}).superRefine((response, context) => {
  if (new Set(response.toolCalls.map((call) => call.callId)).size !== response.toolCalls.length) context.addIssue({ code: "custom", message: "Tool-call identities must be unique." });
  if ((response.finishReason === "tool_calls") !== (response.toolCalls.length > 0)) context.addIssue({ code: "custom", message: "Only a finalized tool-call response may expose executable calls." });
});
export const ModelStreamEventSchema = z.discriminatedUnion("type", [
  ...
  z.strictObject({ type: z.literal("tool_call.delta"), requestId: text, callId: text, name: text.optional(), argumentsDelta: z.string() }),
  ...
  if (event.type === "completed" && event.response.requestId !== event.requestId) context.addIssue({ code: "custom", message: "Completed response identity differs from its envelope." });
});
export const ModelErrorSchema = z.strictObject({
  code: z.enum(["version_mismatch", "invalid_request", "unsupported", "timeout", "aborted", "provider_failure", "unavailable", "incomplete_stream"]),
  requestId: text, message: z.string(),
});

...
export const VerificationIdentitySchema = TargetIdentitySchema.extend({
  ...
  pocInputsDigest: text,
  predicateRevision: text,
  executionPathDigest: text,
});
export const VerifyRequestSchema = z
  .strictObject({
    seamVersion: z.literal(SEAM_VERSION.oracle),
    identity: VerificationIdentitySchema,
    mode: InvestigationModeSchema,
    candidate: FindingSchema,
    pocInputs: z.array(z.string()),
    executionPathRef: text,
  })
  .superRefine((request, context) => {
    if (
      request.candidate.id !== request.identity.candidateId ||
      request.candidate.runId !== request.identity.runId ||
      request.candidate.status !== "candidate" ||
      request.candidate.verification.result !== "pending"
    ) {
      context.addIssue({
        code: "custom",
        message: "Verification requires a pending candidate with matching run and candidate identity.",
      });
    }
  });
...
export const VerifyVerdictSchema = z
  .union([
    z.strictObject({
      ...
      method: z.enum(["flag", "witness", "replay"]),
      artifactRef: text,
    ...
    z.strictObject({
      ...
      reason: z.enum(["unsupported", "insufficient_evidence", "human_replay"]),
  ...
  .superRefine((verdict, context) => {
    ...
    if (
      receipt.authority.kind !== "gyms_oracle" ||
      !["vulnerability", "dependency_exploitability"].includes(receipt.claim) ||
      (receipt.claim === "dependency_exploitability" && verdict.method === "flag") ||
      receipt.runId !== verdict.identity.runId ||
      receipt.findingId !== verdict.identity.candidateId ||
      receipt.candidateRevision !== verdict.identity.candidateRevision ||
      receipt.policy.revision !== verdict.identity.predicateRevision
    ) {
      context.addIssue({
        code: "custom",
        message: "Oracle receipt does not bind a supported runtime claim and predicate.",
      });
    }
  });

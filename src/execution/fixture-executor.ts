...
export class FixtureExecutor {
  constructor(private readonly journal: PostgresJournal, private readonly lease: Lease, private readonly signal: AbortSignal, private readonly hooks: FixtureHooks = {}) {}
  ...
  async execute(callId: string | undefined, name: string, input: unknown): Promise<string> {
    ...
    if (!callId || !tool.success || !args.success) throw new GidorahError("tool_denied", "Only registered fixture tools with finalized IDs and empty arguments can execute.");
    ...
    if (this.hooks.delayBeforeDispatchMs) {
      try { await sleep(this.hooks.delayBeforeDispatchMs, undefined, { signal: this.signal }); }
      catch { throw new GidorahError("stop_requested", "Fixture wait cancelled before dispatch."); }
    }

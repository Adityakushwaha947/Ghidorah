...
import { FIXTURE_MODEL, RUNTIME_VERSION } from "../foundation/contracts.js";
...
const responseSchema = z.strictObject({
  ...
  tool_calls: z.array(z.strictObject({ id: z.string(), name: z.enum(["fixture_increment", "fixture_read"]), args: z.strictObject({}), type: z.literal("tool_call") })).max(1),
  usage_metadata: z.strictObject({ input_tokens: z.literal(1), output_tokens: z.literal(1), total_tokens: z.literal(2) }),
});
...
export class JournaledFixtureModel implements LanguageModelV2 {
  ...
  constructor(private readonly journal: PostgresJournal, private readonly lease: Lease, private readonly guard: BoundaryGuard, private readonly hooks: FixtureHooks = {}) {}
  ...
  private response(options: LanguageModelV2CallOptions) {
    return this.guard(async () => {
      ...
      const response = responseSchema.parse(cached ?? {
        id: `model-${digest({ runId: this.lease.runId, key })}`,
        content: nextTool ? "Executing the registered synthetic fixture." : "Synthetic fixture completed. No security assessment or vulnerability verification was performed.",
        tool_calls: nextTool ? [{ id: `call-${digest({ runId: this.lease.runId, key, ordinal: 0 })}`, name: nextTool, args: {}, type: "tool_call" }] : [],
  ...
  async doGenerate(options: LanguageModelV2CallOptions): Promise<Awaited<ReturnType<LanguageModelV2["doGenerate"]>>> {
    ...
    return {
      content: response.tool_calls.length
        ? response.tool_calls.map((call) => ({ type: "tool-call" as const, toolCallId: call.id, toolName: call.name, input: JSON.stringify(call.args) }))
        : [{ type: "text", text: response.content }],
      finishReason: response.tool_calls.length ? "tool-calls" : "stop",
      usage: { inputTokens: 1, outputTokens: 1, totalTokens: 2 }, warnings: [], response: { id: response.id },
    };
  }
  ...
  async doStream(options: LanguageModelV2CallOptions): Promise<Awaited<ReturnType<LanguageModelV2["doStream"]>>> {
    ...
    return { stream: new ReadableStream<LanguageModelV2StreamPart>({ start(controller) {
      ...
      for (const call of response.tool_calls) controller.enqueue({ type: "tool-call", toolCallId: call.id, toolName: call.name, input: JSON.stringify(call.args) });
      ...
      controller.enqueue({ type: "finish", finishReason: response.tool_calls.length ? "tool-calls" : "stop", usage: { inputTokens: 1, outputTokens: 1, totalTokens: 2 } });
      controller.close();
    } }) };
  }
}

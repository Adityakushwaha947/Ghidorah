...
export type CheckpointStore = { storage: PostgresStore; failureSignal: AbortSignal; assertHealthy: () => void; end: () => Promise<void> };

export function checkpointStore(connection: PoolConfig, initialize = false, lease?: Lease): CheckpointStore {
  if (connection.connectionString && new URL(connection.connectionString).search) throw new GidorahError("database_config", "Checkpoint connections cannot override ownership settings through URL parameters.");
  const pool = new Pool({ ...connection, options: checkpointConnectionOptions(lease), max: 2, connectionTimeoutMillis: 8000, statement_timeout: 10000, application_name: "gidorah-mastra-checkpoints" });
  ...
  return { storage, failureSignal: workflows.failureController.signal, assertHealthy: () => workflows.assertHealthy(), end: () => pool.end() };
}

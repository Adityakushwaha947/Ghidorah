...
async function crashAt(point: string): Promise<string> {
  const child = spawn(process.execPath, [new URL("../helpers/crash-worker.ts", import.meta.url).pathname, point], {
    cwd: process.cwd(),
    env: process.env,
  ...
  assert.ok(
    lines.some((line) => line.faultPoint === point),
    `The requested crash boundary was not reached: ${output}`,
  );
...
test("recovery after model, intent and result commits does not duplicate the fixture effect", async () => {
  for (const boundary of ["after-model", "after-intent", "after-result"]) {
    ...
    try {
      ...
      assert.equal(run.spent.steps, 5);
      assert.equal(run.spent.tokens, 6);
      const counter = await journal.pool.query("SELECT counter FROM gidorah_mastra.fixture_targets WHERE run_id=$1", [
        runId,
      ]);
...
test("artifact tampering blocks completed-action replay", async () => {
  ...
  assert.ok(ref);
  await journal.pool.query("UPDATE gidorah_mastra.artifacts SET bytes='tampered fixture' WHERE run_id=$1 AND ref=$2", [
    runId,
    ref,
  ]);
  const backend = new GidorahBackend(connection);

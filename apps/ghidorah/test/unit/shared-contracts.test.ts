...
import { z } from "zod";
import * as contracts from "@ghidorah/contracts";
import { fixtureConfig, validateFixtureRun, FIXTURE_TARGET } from "@ghidorah/foundation";
import { validateOracleVerdict, validateVerificationRequest } from "@ghidorah/foundation";
...
test("shared wire definitions do not import the backend, Node, PostgreSQL or Mastra", async () => {
  const root = new URL("../../../../packages/contracts/src/", import.meta.url).pathname;
...
test("shared RunConfig rejects duplicate capabilities and PRs without pinned change context", () => {
  ...
  assert.equal(
    contracts.RunConfigSchema.parse(
      fixtureConfig({
...
test("candidate and snapshot contracts preserve claim, evidence and independent receipts", () => {
  ...
  const snapshot = {
    ...
    spent: { tokens: 10, steps: 1, wallSec: 1 },
    findings: [confirmed],
    installDecisions: [],
    pendingApprovals: [],
    pendingReviews: [],
    terminal: { outcome: "completed", cleanupOk: true },
  };
...
test("review snapshots bind the same inconclusive finding and evidence revision", () => {
  ...
  const snapshot = {
    ...
    capabilities: [finding.capability],
    caps: { tokens: 100, steps: 10, wallSec: 60 },
    spent: { tokens: 1, steps: 1, wallSec: 1 },
    findings: [finding],
    installDecisions: [],
    pendingApprovals: [],
  ...
  for (const pendingReviews of [
    [{ ...review, findingId: "other" }],
    [{ ...review, evidenceRev: "old" }],
    [review, { ...review, reviewRequestId: "another" }],
  ]) {
...
test("finding updates cannot assert confirmation without proof or use another run's receipt", () => {
  ...
  const event = {
    contractVersion: "1.0.0",
    runId: proof.identity.runId,
    seq: 2,
    type: "finding.update",
    findingId: proof.identity.candidateId,
    status: "confirmed",
    verification: { result: "pass", method: proof.method, artifactRef: proof.artifactRef, receipt: proof.receipt },
  };
  ...
  assert.throws(() =>
    contracts.EventSchema.parse({ ...event, verification: { result: "pending", receipt: proof.receipt } }),
  );
});
...
test("Oracle request validation binds captured candidate, PoC inputs and execution bytes", () => {
  ...
  assert.throws(() =>
    validateVerificationRequest(
      { ...request, candidate: { ...request.candidate, title: "changed" } },
      "synthetic execution",
    ),
  );
  assert.throws(() => validateVerificationRequest({ ...request, seamVersion: "2.0.0" }, "synthetic execution"), {
    code: "version_mismatch",
  });
  assert.throws(() =>
    validateVerificationRequest(
      { ...request, candidate: { ...request.candidate, runId: "other" } },
      "synthetic execution",
    ),
  );
});
...
for (const key of Object.keys(verificationRequest().identity)) {
  test(`Oracle rejects a verdict with changed ${key}`, () => {
    ...
    assert.throws(() =>
      validateOracleVerdict(verificationRequest(), {
        ...response,
        identity: { ...response.identity, [key]: "changed" },
      }),
    );
    assert.throws(() =>
      validateOracleVerdict(verificationRequest(), {
        ...
        result: "needs_human",
        reason: "unsupported",
        detail: "Synthetic inconclusive check",
      }),
    );
  });
}
...
test("Oracle receipts cannot substitute another claim, predicate, subject or evidence", () => {
  ...
  for (const mutation of [
    { claim: "advisory_match" },
    { evidenceRev: "other" },
    { subjectDigest: "other" },
    { policy: { id: "fixture", revision: "other" } },
    { authority: { kind: "human", id: "other" } },
  ]) {
    assert.throws(() =>
      validateOracleVerdict(verificationRequest(), { ...response, receipt: { ...response.receipt, ...mutation } }),
    );
  }
});
...
test("Oracle errors stay errors, distinct from fail and needs_human verdicts", () => {
  ...
  const response = {
    ...
    result: "needs_human",
    reason: "unsupported",
    detail: "Predicate unavailable for this claim",
  };
...
test("registry identity and expiry are mandatory; caller allowlists are not eligibility", () => {
  ...
  assert.ok(
    contracts.RangeEligibilitySchema.safeParse({
      eligible: true,
      identity,
      endpoint: "fixture://instance",
      expiresAt: "2026-09-20T00:00:00Z",
    }).success,
  );
  assert.throws(() =>
    contracts.RangeEligibilitySchema.parse({ eligible: true, identity, endpoint: "fixture://instance" }),
  );
  assert.throws(() =>
    contracts.RangeEligibilitySchema.parse({ eligible: true, scopeAllowlist: ["https://example.invalid"] }),
  );
  assert.deepEqual(contracts.RangeEligibilitySchema.parse({ eligible: false, reason: "not registered" }), {
    eligible: false,
...
test("model contract validates full tool-result linkage and unique tool/call IDs", () => {
  ...
  for (const messages of [
    ...
    [assistant, { ...result, callId: "other" }],
    [assistant, result, assistant, result],
    [assistant, { role: "user", content: "skip result" }, result],
  ]) {
...
test("model schemas reject unsupported fields, nonfinite settings, missing usage and unsafe completion shapes", () => {
  ...
  for (const fields of [
    ...
    { finishReason: "refusal", toolCalls: [{ callId: "call", name: "fixture", arguments: {} }] },
  ]) {
    assert.throws(() => contracts.ModelResponseSchema.parse({ ...modelResponse(), ...fields }));
  }
  assert.throws(() =>
    contracts.ModelStreamEventSchema.parse({ type: "completed", requestId: "different", response: modelResponse() }),
  );
});

...
import {
  ...
  prepareBundle,
  replacements,
} from "../../scripts/mastra-recovery-patch.mjs";

const require = createRequire(import.meta.url);
const engines = [
  {
    name: "ESM",
    workflows: await import("@mastra/core/workflows"),
    agent: await import("@mastra/core/agent"),
    durable: await import("@mastra/core/agent/durable"),
  },
  {
    name: "CommonJS",
    workflows: require("@mastra/core/workflows"),
    agent: require("@mastra/core/agent"),
    durable: require("@mastra/core/agent/durable"),
  },
];
...
for (const { name, workflows, agent, durable } of engines) {
  ...
  test(`${name}: restart uses only the active step's saved input; fresh, fallback and resume semantics remain intact`, async () => {
    class CaptureEngine extends workflows.DefaultExecutionEngine {
      async executeStep(parameters) {
        return {
          result: { status: "success", output: parameters.prevOutput },
          stepResults: parameters.stepResults,
          mutableContext: this.buildMutableContext(parameters.executionContext),
        };
      }
      async executeMapping(parameters) {
        return this.executeStep(parameters);
      }
    ...
    for (const type of ["step", "mapping", "agent", "tool"]) {
      for (const scenario of [
        ...
        const stepResults = {
          input: {},
          previous: { status: "success", output: previous },
          active: {
            status: scenario === "terminal-restart" ? "success" : "running",
            ...(scenario === "missing-payload"
              ? {}
              : { payload: scenario === "undefined-payload" ? undefined : saved }),
          },
        };
        const restart = ["fresh", "resume"].includes(scenario)
          ? undefined
          : { activeStepsPath: scenario === "inactive" ? {} : { active: [1] }, activePaths: [1], stepResults };
        const result = await engine.executeEntry({
          workflowId: "fixture",
          runId: "fixture-run",
          entry,
          prevStep: { type: "step", step: { id: "previous" } },
          stepResults,
          restart,
        ...
        const expected = ["fresh", "inactive", "missing-payload"].includes(scenario)
          ? previous
  ...
  test(`${name}: pruning retains model output needed after tool recovery without retaining all history`, () => {
    const fixture = durable.createDurableAgent({
      agent: new agent.Agent({
        id: "pruning-regression",
        name: "Pruning regression",
        model: { specificationVersion: "v2", provider: "fixture", modelId: "fixture", supportedUrls: {} },
      }),
    });
    const prune = fixture.getWorkflow().options.pruneSnapshot;
    const state = {
      messageListState: { messages: ["assistant tool call"] },
      accumulatedSteps: ["prior turn"],
      stepResult: { reason: "tool-calls", request: "must not retain" },
    };
    const snapshot = {
      runId: "fixture-run",
      status: "running",
      activePaths: [3],
      activeStepsPath: { "durable-tool-call": [3] },
      context: {

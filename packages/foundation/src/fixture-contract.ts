...
export type { AgentControl, Budget, ContractContext, RunConfig, Terminal } from "@ghidorah/contracts";
...
const targetFields = {
  target: z.literal(FIXTURE_TARGET),
  mode: z.literal("pentest"),
  capabilities: z.tuple([z.literal("agentic_pentesting")]),
  caps: BudgetSchema,
};
...
export const FixtureEventSchema = z.discriminatedUnion("type", [
  ...
  z.strictObject({
    ...
    ...targetFields,
    spent: BudgetSchema,
    findings: z.array(z.never()),
    installDecisions: z.array(z.never()),
    pendingApprovals: z.array(z.never()),
    pendingReviews: z.array(z.never()),
  ...
  z.strictObject({
    ...
    needsHuman: z.literal(0),
    discarded: z.literal(0),
  }),
]);
export type FixtureEvent = z.infer<typeof FixtureEventSchema>;
export type EventPayload = FixtureEvent extends infer Variant
  ? Variant extends FixtureEvent
    ? Omit<Variant, "contractVersion" | "runId" | "seq">
    : never
  : never;

export type RunHandle = {
  runId: string;
  events: AsyncIterable<FixtureEvent>;
  control: (control: AgentControl) => void;
};
...
export function validateFixtureRun(target: string, input: unknown): RunConfig {
  ...
  if (!result.success)
    throw new GidorahError("invalid_config", "Run configuration does not match the supported contract.");
  const config = result.data;
  if (Object.hasOwn(config, "capUsd")) throw new GidorahError("unsupported_budget", "USD metering is not implemented.");
  if (
    target !== FIXTURE_TARGET ||
    config.model !== FIXTURE_MODEL ||
    config.approvalProfile !== "closed-world" ||
    config.targetKind !== "web" ||
    config.change !== undefined ||
    (config.capabilities !== undefined &&
      (config.capabilities.length !== 1 || config.capabilities[0] !== "agentic_pentesting"))
  ) {
    throw new GidorahError(
      "unsupported_profile",
      "Only the explicit local counter fixture and fixture model are implemented. Live targets are disabled.",
    );
  }
  if (
    !config.authorization.asserted ||
    config.authorization.scopeAllowlist.length !== 1 ||
    config.authorization.scopeAllowlist[0] !== FIXTURE_TARGET
  ) {
...
export function fixtureConfig(overrides: Partial<RunConfig> = {}): RunConfig {
  return {
    ...
    capTokens: 1000,
    capWallSec: 7200,
    approvalProfile: "closed-world",
    authorization: { asserted: true, scopeAllowlist: [FIXTURE_TARGET] },
    ...overrides,
  };
}

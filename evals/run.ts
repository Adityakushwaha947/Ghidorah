...
import { CONTRACT_VERSION, RUNTIME_VERSION } from "../src/foundation/fixture-contract.js";
...
try {
  for await (const event of stream) {
    ...
    const status: CaseResult["status"] = skipped
      ? "not_run"
      : event.type === "test:pass"
        ? "passed"
        : assertionFailure(event.data.details.error)
          ? "failed"
          : "error";
    const reason =
      status === "passed"
...
const unchanged =
  digest(sourceFiles) === digest(await treeFingerprint("src")) &&

...
export async function managePatch({ directory = coreDirectory, check = false } = {}) {
  ...
  for (const bundle of bundles) {
    ...
    if (check && content !== patched)
      throw new Error("Mastra recovery patch is missing. Run npm run patch:mastra before execution.");
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    if (process.argv.slice(2).some((argument) => argument !== "--check"))
      throw new Error("Usage: node scripts/mastra-recovery-patch.mjs [--check]");

...
const fingerprint = {
  ...
  canonicalEncoderSha256: sha256(await readFile(resolve("src/foundation/digest.ts"), "utf8")),
...
if (mode === "--fingerprint") process.stdout.write(`${JSON.stringify(fingerprint, null, 2)}\n`);
  ...
  if (canonicalJson(expected) !== canonicalJson(fingerprint))
    throw new Error(
      "Shared contract fingerprint changed. Review compatibility and versioning before updating contracts/manifest.json.",
    );
  if (mode === "--emit") {
